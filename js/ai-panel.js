...
// itself never calls a model — see docs/adr/0007-local-helper-connects-any-ai.md.
...
(function () {
  ...
  //
  // A page can't start programs, but it can follow an inspiration-library://
  ...
  var START_LINK = 'inspiration-library://start';
  var WAIT_MS = 90000;
  ...
  function openStartHelper() {
    helperUp().then(function (up) {
      ...
      var status = el('p', { class: 'add-modal-status', 'aria-live': 'polite', text: 'Starting Inspiration Library…' });
      ...
      mount([
        ...
        el('p', { class: 'ai-lede', text: 'Starting the library’s companion app, which talks to your AI. If your browser asks to open “Inspiration Library Helper”, choose Open (and tick “Always allow” so it won’t ask again).' }),
        status,
        hint,
      ...
      function start() {
        ...
        status.textContent = 'Starting Inspiration Library…';
        ...
        (function poll() {
          ...
          helperUp().then(function (ok) {
            ...
            if (waited > 8000 && hint.hidden) {
              hint.hidden = false;
              hint.textContent = 'The very first start sets things up and can take a minute. Nothing happening at all? The first time only, open this folder and double-click “Start Inspiration Library” (the .bat file on Windows). After that, this button starts it for you.';
            }
            if (waited > WAIT_MS) {
