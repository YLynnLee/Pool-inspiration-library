...
// access — see docs/adr/0002-app-writes-its-own-data.md. Loadable both as a
...
// See docs/adr/0002-app-writes-its-own-data.md: the data file's shape is a
...
// status/draftReason are optional (ticket 08) and only emitted when present,
...
// The one function ADR-0002 calls the risky part of the whole decision.
...
// array — used both by purge (this ticket) and by the drain (ticket 09)
...
// Same contract as the library: ADR-0002's risky-part promise, and the same
...
var CATEGORIES_HEADER =
  [
    ...
    '//',
    '// Deliberately small. The spec commits to re-cutting this taxonomy once',
...
// array — the seam a drain uses to add a category (ticket 04 onward).
...
// parked (ticket 13). Every other entry passes through unchanged, by
...
// Same ADR-0002 contract as the library and categories files. Entry shape
...
// Canonical section order, per the spec's "Section Order" list. Google
...
// The one function this ticket exists to write: entry -> DESIGN.md text.
