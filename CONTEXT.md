...
The vocabulary this project uses. Terms here are the canonical ones — code and docs should use
these words and avoid the synonyms noted against each.

This file is a glossary and nothing else.
...
collector by hand, never by a drain.
...
capture, not the analysis or the category. A drain never parks for a categorisation reason.
...
Pool" (or `npm start`) that serves the app
on `localhost` and runs drains on the Drain panel's behalf, either by launching the collector's
agent CLI or by calling a model API directly. See `SECURITY.md`.
