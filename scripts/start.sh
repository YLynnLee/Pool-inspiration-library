#!/bin/sh
# Starts the Inspiration Library helper. Sets itself up on the first run, so
# nobody has to type `npm install`. See docs/adr/0007-local-helper-connects-any-ai.md.
#
#   scripts/start.sh                     what "Start Inspiration Library" runs:
#                                        in a Terminal window, opens the app
#   scripts/start.sh --background [--open]
#                                        what the app's Connect AI button
#                                        triggers (via inspiration-library://):
...
if ! command -v node >/dev/null 2>&1; then
  ...
  echo "Opening the download page — install it, then start Inspiration Library again."
...
sh scripts/register-url-handler.sh || echo "(Couldn't register the inspiration-library:// link; Connect AI will ask you to start the helper by hand.)"

if [ "$OPEN" -eq 1 ]; then
  [ "$BACKGROUND" -eq 0 ] && echo "Starting Inspiration Library. Keep this window open while you use the Drain button."
