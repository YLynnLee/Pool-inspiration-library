...
function unixScript(action) {
  return [
    ...
    'clear',
    'printf "%s\\n\\n%s\\n\\n" ' + shQuote('Inspiration Library — ' + action.title) + ' ' + shQuote(action.steps),
    action.command,
...
function winScript(action) {
  return [
    '@echo off',
    'cd /d "' + ROOT + '"',
    cmdEcho('Inspiration Library - ' + action.title),
    'echo.',
...
async function openTerminal(action) {
  ...
  if (process.platform === 'win32') {
    var cmdFile = path.join(os.tmpdir(), 'inspiration-library-' + stamp + '.cmd');
  ...
  var file = path.join(os.tmpdir(), 'inspiration-library-' + stamp + (process.platform === 'darwin' ? '.command' : '.sh'));
