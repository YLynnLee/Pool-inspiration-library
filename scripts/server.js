...
// See docs/adr/0007-local-helper-connects-any-ai.md.
//
...
// the files the app writes itself (docs/adr/0002-app-writes-its-own-data.md).
...
function serveStatic(req, res, pathname) {
  var rel = pathname === '/' ? 'index.html' : decodeURIComponent(pathname.replace(/^\/+/, ''));
...
server.listen(opts.port, '127.0.0.1', function () {
  console.log('Inspiration Library is running at ' + APP_URL);
...
server.on('error', function (err) {
  ...
  http.get({ host: '127.0.0.1', port: opts.port, path: '/api/status', headers: { host: 'localhost:' + opts.port } }, function (res) {
    ...
    if (res.statusCode === 200) {
      console.log('Inspiration Library is already running at ' + APP_URL);
      if (opts.open) openApp();
    } else {
