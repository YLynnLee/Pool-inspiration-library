...
Nothing in this repo depends on a particular agent; see `docs/adr/0006-agent-agnostic-drain.md`.
...
Double-clicking "Start Inspiration Library" (`.command` on macOS, `.bat` on Windows; both run
...
`index.html` page starts the helper itself through an `inspiration-library://` link the launcher
...
default. See `docs/agents/nightly-drain.md` for setup and `docs/adr/0003-launchd-scheduling.md` for why `launchd` is the mechanism.

## Agent skills

### Issue tracker

Issues are tracked as local markdown files under `.scratch/<feature-slug>/`. See
`docs/agents/issue-tracker.md`.

### Triage labels

The five canonical triage roles, used with their default label strings, recorded as a `Status:`
