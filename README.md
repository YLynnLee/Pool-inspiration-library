...
├── Start Pool.command / .bat   # Double-click: set up, start helper, open app
...
1. Double-click **Start Pool**, or just open `index.html` in a Chromium-based browser to browse (required for File System Access API).
...
1. Double-click **Start Pool** in this folder (`.command` on Mac, `.bat` on
...
`pool://` link. From then on, opening `index.html` and clicking **Connect AI** starts
everything by itself. Your browser asks once whether to open "Pool Helper"; choose
