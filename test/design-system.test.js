...
// google-labs-code/design.md (fetched via `gh api` while implementing this
// ticket; not vendored into the repo). Exercises every YAML-bearing field
// this serialiser supports in combination, then actually lints the output
// with the real CLI, per this ticket's acceptance criterion.
...
test('serializeDesignMd: the atmospheric-glass fixture round-trips to a lint-clean DESIGN.md', () => {
  ...
  try {
    ...
    try {
      ...
      if (spawnError.code === 'ENOENT' || /ENOTFOUND|ETIMEDOUT|ECONNREFUSED/.test(String(spawnError))) {
