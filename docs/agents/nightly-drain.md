...
never set up. See `docs/adr/0003-launchd-scheduling.md` for why `launchd` is the mechanism.
...
2. Otherwise it runs the agent CLI named by `DRAIN_AGENT_CMD` (see **Choosing the agent** below;
   ...
   the guards on creating a category (see `docs/adr/0004-drains-always-resolve.md`), and malformed
...
2. Copy `scripts/com.inspiration-library.nightly-drain.plist` to `~/Library/LaunchAgents/`.
3. In the copy, replace both `REPO_PATH_HERE` placeholders with the absolute path to this
   checkout (e.g. `/Users/you/Desktop/Inspiration Library`).
4. Load it:
   ```
   launchctl load ~/Library/LaunchAgents/com.inspiration-library.nightly-drain.plist
   ```
5. To stop it, unload the same path and optionally delete the copied plist:
   ```
   launchctl unload ~/Library/LaunchAgents/com.inspiration-library.nightly-drain.plist
   ```
