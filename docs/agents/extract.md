...
DESIGN.md, harvest, extract — and `docs/adr/0005-design-systems-follow-design-md.md` for why the
format is Google's, unextended). It is invoked standalone (the `extract` command, or `prompts/extract.md` pasted into any agent),
or as a step inside a drain (`docs/agents/drain.md`, from ticket 07 onward). Like the drain, any
agent that can run a shell, read and write files, and look at images can run it — the browser work
and the file write are scripts (`docs/adr/0006-agent-agnostic-drain.md`).
...
would just relocate the noise (`docs/adr/0005`'s "Alternatives considered"). Turn the raw
...
- **Colors** names each swatch humanly against its semantic key (`js/curation.js`'s
  ...
  prose, per `docs/adr/0005`'s "Refero's additions survive as prose, not as schema").
...
to read (`docs/adr/0005`'s "Alternatives considered" explains why sidecars aren't the canonical
store). Offline, `--skip-lint` bypasses the linter; say so in the report.
...
`docs/adr/0005`'s write-order extension: category, then reference, then design system, then inbox
line.
...
that term means screenshot failure and nothing else (`docs/adr/0004-drains-always-resolve.md`).
