...
the agent's half is looking at screenshots and writing one JSON file per capture.
...
installed, `npx playwright install chromium`). Double-clicking "Start Pool" does the
install itself.
